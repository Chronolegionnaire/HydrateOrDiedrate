    ...
    public static class HydrationConfigLoader
    {
        public static List<JObject> LoadHydrationPatches(ICoreAPI api)
        {
            ...
            if (File.Exists(configPath))
            {
                ...
                var patches = JArray.Parse(json).ToObject<List<JObject>>();
                return patches;
            }
        ...
        public static void GenerateDefaultHydrationConfig(ICoreAPI api)
        {
            ...
            if (!File.Exists(configPath))
            {
                var defaultConfig = new List<JObject>
                {
                    new JObject
                    {
                ...
                File.WriteAllText(configPath, JArray.FromObject(defaultConfig).ToString());
            }
        }
    }
}
