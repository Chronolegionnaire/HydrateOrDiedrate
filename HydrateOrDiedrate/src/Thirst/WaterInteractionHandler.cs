...
namespace HydrateOrDiedrate
{
    public class WaterInteractionHandler
    {
        ...
        private readonly Dictionary<string, PlayerDrinkData> playerDrinkData = new Dictionary<string, PlayerDrinkData>();
        ...
        public void CheckPlayerInteraction(float dt, IServerPlayer player)
        {
            ...
            if (IsAqueduct(block) || IsFurrowedLand(block))
            {
                block = HandleFluidBlock(blockSel.Position, block) ?? block;
                if (block == null)
                {
                    StopDrinking(player, drinkData);
                    return;
            ...
            HandleDrinkingStep(player, blockSel, currentTime, collectible, drinkData, hydrationValue);
        }

        private CollectibleObject GetCollectibleObject(Block block)
        {
            return block == null ? null : _api.World.Collectibles?.Find(c => c.Code.Path == block.Code.Path);
        }

        private bool IsAqueduct(Block block)
        {
            return block?.GetType().GetInterface("IAqueduct") != null;
        }

        private bool IsFurrowedLand(Block block)
        {
            return block?.Code?.Path != null && block.Code.Path.StartsWith("furrowedland");
        }

        private Block HandleFluidBlock(BlockPos pos, Block block)
        {
            var fluidBlock = _api.World.BlockAccessor.GetBlock(pos, BlockLayersAccess.Fluid);
        ...
        private void HandleDrinkingStep(IServerPlayer player, BlockSelection blockSel, long currentTime, CollectibleObject collectible, PlayerDrinkData drinkData, float hydrationValue)
        {
            ...
            if (healthEffect != 0)
            {
                ...
                player.Entity.ReceiveDamage(damageSource, Math.Abs(healthEffect));
            }
            ...
            if (blockSel != null && _api.World.BlockAccessor.GetBlock(blockSel.Position).Code.Path.StartsWith("wellwater"))
            {
                var blockBehavior = _api.World.BlockAccessor.GetBlock(blockSel.Position).GetBehavior<BlockBehaviorWellWaterFinite>();
                var naturalSourcePos = blockBehavior?.FindNaturalSourceInLiquidChain(_api.World.BlockAccessor, blockSel.Position);
        ...
        private BlockSelection RayCastForFluidBlocks(IServerPlayer player)
        {
            ...
            Vec3d direction = toPos.Sub(fromPos).Normalize();
            Vec3d step = direction.Mul(0.1);
            double maxDistanceSq = 5 * 5;
            ...
            for (int i = 0; i < 5 / 0.1; i++)
            {
                ...
                if (block == null)
                {
                    currentPos = fromPos.Add(step.Mul(i + 1));
                    continue;
                }
                ...
                currentPos = fromPos.Add(step.Mul(i + 1));
                if (currentPos.SquareDistanceTo(fromPos) > maxDistanceSq) break;
            }
            return null;
        }
