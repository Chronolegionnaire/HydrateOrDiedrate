...
public sealed class DrinkHudOverlayRenderer : IRenderer
{
    ...
    private const double DrinkDurationMs = 1000; //TODO config
    ...
    private void UpdateCircleMesh(float progress)
    {
        ...
        if (progress == 1.0f)
        {
            data.AddIndices([(steps * 2) - 2, (steps * 2) - 1, 0]);
            data.AddIndices([0, (steps * 2) - 1, 1]);
        }
