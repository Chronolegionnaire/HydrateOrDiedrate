...
{
    [HarmonyPatch]
    public static class COABlockCookingContainerPatch
    {
        static bool Prepare()
        {
            return AppDomain.CurrentDomain.GetAssemblies().Any(a => a.GetName().Name == "CoreOfArtsPatch");
        }
        static MethodBase TargetMethod()
        {
            var assembly = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name == "CoreOfArts");
            if (assembly == null) return null;

            var type = assembly.GetType("CoreOfArts.Blocks.COABlockCookingContainer");
        ...
        static bool Prefix(IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemSlot inputSlot, ItemSlot outputSlot, object __instance)
        {
            var instanceType = __instance.GetType();
            var getCookingStacksMethod = instanceType.GetMethod("GetCookingStacks", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            ...
            var getMatchingCookingRecipeMethod = instanceType.GetMethod("GetMatchingCookingRecipe", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
